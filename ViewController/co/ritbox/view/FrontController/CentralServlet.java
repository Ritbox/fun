package co.ritbox.view.FrontController;

import co.ritbox.view.FrontController.Controller;
import co.ritbox.view.FrontController.WebConfig;
...
import co.ritbox.view.FrontController.ControllerConfig;
import co.ritbox.view.FrontController.View;

import java.util.Set;

final public class CentralServlet extends HttpServlet {
    ...
    private void doProcess(HttpServletRequest request,
        ...
        final String actionPath =
            servletPath.substring(1, servletPath.lastIndexOf("."));
        final String fileFormat = servletPath.substring(servletPath.lastIndexOf(".")+1, servletPath.length());
        final Map controllers = webConfig.getControllers();
        final ControllerConfig ctrConfig =
            (ControllerConfig)controllers.get(actionPath);
        ...
        if (ctrConfig != null) {
            ...
            throw new UnsupportedOperationException(format("action %s is not found in mapping file.",
                                                           actionPath));
    }
    ...
    private void prepareModelData(HttpServletRequest request, View view) {
        ...
        if (model != null) {
            for (Entry data : (Set<Entry>)model.entrySet()) {
                request.setAttribute(String.valueOf(data.getKey()),
                                     data.getValue());
            }
        }
    }
